...
function handleVerticalResize(event) {
  ...
  const handlerLeft = Math.max(
    0,
    Math.min(
      containerWidth - handlerVerticalWidth,
      handlerVertical.offsetLeft + diffX
    )
  );
  ...
  // Update the chart container width
  document.getElementById("resizable-chart").style.width = `${item2Width}px`;
...
function handleHorizontalResize(event) {
  ...
  const handlerTop = Math.max(
    0,
    Math.min(
      containerHeight - handlerHorizontalHeight,
      handlerHorizontal.offsetTop + diffY
    )
  );
  ...
  const placeholder2Height =
    containerHeight - handlerHorizontalHeight - placeholder1Height;

  document.querySelector(".placeholder1").style.flexBasis = `${
    (placeholder1Height / containerHeight) * 100
  }%`;
  document.querySelector(".placeholder2").style.flexBasis = `${
    (placeholder2Height / containerHeight) * 100
  }%`;

  document.getElementById(
    "resizable-chart"
  ).style.height = `${placeholder2Height}px`;
